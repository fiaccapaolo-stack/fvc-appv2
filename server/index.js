...
const VAPID_KEY = 'fvc:v2:vapid_keys';
...
async function setupVapid() {
  ...
  vapidKeys = typeof stored === 'string' ? JSON.parse(stored) : stored;
  if (!vapidKeys?.publicKey || !vapidKeys?.privateKey) {
    vapidKeys = webpush.generateVAPIDKeys();
    await redis.set(VAPID_KEY, vapidKeys);
    console.log('Nuove chiavi VAPID della v2 create e salvate in Upstash.');
  }
  webpush.setVapidDetails(process.env.VAPID_SUBJECT || 'mailto:negozio@example.com', vapidKeys.publicKey, vapidKeys.privateKey);
}
...
setupVapid()
  .then(() => app.listen(process.env.PORT || 3000, () => console.log('FVC App v2 avviata')))
  .catch((error) => { console.error('Impossibile inizializzare le notifiche:', error); process.exit(1); });
